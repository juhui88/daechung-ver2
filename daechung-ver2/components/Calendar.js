  ...
  const buildCalendar = () => {
    ...
    const dateEndWeek = date.clone().endOf('month').week() === 1 ? 53 : date.clone().endOf('month').week();
     ...
     for (let week = dateStartWeek; week <= dateEndWeek; week++) {
      calendar.push(
        <div key={week} className="grid grid-cols-7 items-center justify-items-center gap-10 ">
        {[Array(7).fill(0).map( (n,i) => {
          ...
          return <span key={i} className={cls("text-center hover:font-extrabold text-lg px-1", isMonth ? "text-gray-200": "", !isMonth&& isToday ? " bg-[#bed0d9] rounded-full" :"")} >{current.format("D")}</span>
        })]}  
        </div>
